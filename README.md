<!DOCTYPE html>
<html lang="ru" data-theme="dark" data-accent="green" data-font="mono" data-density="normal" data-bgfx="default">
<head>
<meta charset="UTF-8">
...
<style>
  ...
  html[data-theme="light"] {
    --bg: #f4f4f0; --panel: #ffffff; --panel-2: #fafafa;
    --border: #e0e0e0; --text: #333333;
  }
  ...
  html[data-font="mono"] { --font-stack: 'Consolas', 'Courier New', monospace; }
  html[data-font="cyber"] { --font-stack: 'Impact', 'Arial Black', sans-serif; }
  html[data-density="compact"] { --grid-gap: 8px; }
  html[data-density="normal"] { --grid-gap: 14px; }
  html[data-density="spacious"] { --grid-gap: 24px; }
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  html[data-bgfx="grid"] body::before {
    opacity: .35;
    background-image:
  ...
  html[data-bgfx="noise"] body::before {
    opacity: .5;
  ...
  html[data-bgfx="gradient"] body::before {
    opacity: .9;
    background:
      radial-gradient(circle at 20% 20%, rgba(var(--accent-rgb), .12), transparent 55%),
      radial-gradient(circle at 80% 70%, rgba(var(--accent-rgb), .09), transparent 55%),
      radial-gradient(circle at 50% 110%, rgba(var(--accent-rgb), .14), transparent 60%);
  }
  html[data-bgfx="space"] body::before {
    opacity: .7;
    background:
      ...
      radial-gradient(1.5px 1.5px at 40% 80%, rgba(var(--accent-rgb),.9), transparent),
      radial-gradient(1px 1px at 90% 20%, rgba(255,255,255,.5), transparent),
      radial-gradient(1.5px 1.5px at 10% 90%, rgba(var(--accent-rgb),.8), transparent),
  ...
  .parallax-layer.back .parallax-line { font-size: 5em; color: rgba(var(--accent-rgb), 0.028); }
  ...
  .watermark-overlay .wm {
    position: absolute; color: rgba(var(--accent-rgb), 0.045);
    font-weight: bold; font-size: 1.5em; letter-spacing: 5px; white-space: nowrap;
    transform: rotate(-35deg); user-select: none; pointer-events: none; text-transform: uppercase;
  }
  ...
  html[data-theme="light"] .sidebar { background: linear-gradient(180deg, #ffffff, #f0f0eb); }
  ...
  .sidebar-logo .classif {
    display: inline-block; background: #ff2a2a; color: #fff;
  ...
  @keyframes levelGlitch {
    0%, 86%, 100% { text-shadow: 0 0 30px rgba(255,0,0,.45); transform: translate(0,0); }
    87% { text-shadow: -2px 0 var(--accent), 2px 0 #00ccff, 0 0 30px rgba(255,0,0,.9); transform: translate(-1px,0); }
    89% { text-shadow: 2px 0 #ff0000, -2px 0 var(--accent), 0 0 30px rgba(255,0,0,.9); transform: translate(1px,1px); }
    91% { text-shadow: -2px 0 #00ccff, 2px 0 #ff0000, 0 0 30px rgba(255,0,0,.9); transform: translate(-1px,-1px); }
    93% { text-shadow: 2px 0 var(--accent), -2px 0 #00ccff, 0 0 30px rgba(255,0,0,.9); transform: translate(1px,0); }
    95%, 97% { text-shadow: 0 0 30px rgba(255,0,0,.45); transform: translate(0,0); }
  }
  ...
  .nav-section-header .arrow { font-size: .7em; transition: transform .3s var(--ease); color: var(--accent); }
  ...
  .main-inner { width: var(--content-width); max-width: 100%; margin: 0 auto; }
  ...
  .search-icon {
    ...
    transform: translateY(-50%); font-size: .95em; pointer-events: none; opacity: .55; z-index: 2;
  }
  .search-box input {
    width: 100%; padding: 12px 42px 12px 44px;
    background: rgba(8,8,8,.92);
    border: 2px solid var(--border); border-radius: 10px;
    color: var(--accent); font-family: inherit; font-size: .85em;
  ...
  .search-clear {
    ...
    border: none; cursor: pointer; font-size: .8em;
  ...
  .search-tag {
    display: inline-block; color: var(--accent-dim); font-size: .72em;
    letter-spacing: 1.5px; text-transform: uppercase;
    margin-right: 8px; padding: 1px 6px;
    background: rgba(var(--accent-rgb), .08); border-radius: 3px; font-weight: bold;
  }
  .search-item mark { background: rgba(var(--accent-rgb), .28); color: var(--accent); padding: 1px 3px; border-radius: 3px; }
  .search-empty { padding: 22px 16px; text-align: center; color: #555; font-size: .82em; }
  @media (max-width: 900px) {
    .search-box { top: 15px; right: 15px; width: calc(100% - 90px); max-width: 340px; }
    .search-box input { padding: 10px 38px; font-size: .78em; }
  }

  /* ============ КНОПКА АДМИНКИ (выше поиска) ============ */
  .admin-open-btn {
    position: fixed; top: 20px; right: 380px;  /* левее поиска на его ширину + зазор */
  ...
  @keyframes pulseUnlock {
    0%, 100% { box-shadow: 0 0 20px rgba(var(--accent-rgb), .5); }
    50% { box-shadow: 0 0 40px rgba(var(--accent-rgb), 1); }
  }
  @media (max-width: 900px) {
    .admin-open-btn { top: 15px; right: 340px; width: 38px; height: 38px; }
  }
  @media (max-width: 640px) {
    .admin-open-btn { right: auto; left: 15px; top: 65px; }
  }
  ...
  .modal {
    ...
    box-shadow: 0 0 60px rgba(var(--accent-rgb), .35);
    text-align: center;
  }
  ...
  .modal-btn {
    ...
    font-family: inherit; font-weight: bold; letter-spacing: 1.5px; font-size: .82em;
    transition: .25s;
  }
  ...
  .modal-btn.ghost:hover { color: #ccc; border-color: #666; }
  ...
  html[data-theme="light"] .admin-panel { background: linear-gradient(180deg, #ffffff, #f4f4f0); }
  ...
  .admin-head h3 { color: var(--accent); font-size: 1em; letter-spacing: 3px; text-transform: uppercase; }
  .admin-head .actions { display: flex; gap: 6px; }
  .admin-head button {
    background: rgba(255,255,255,.05); border: 1px solid var(--border);
    color: #888; border-radius: 6px; padding: 6px 10px;
    cursor: pointer; font-family: inherit; font-size: .75em; transition: .2s;
  }
  .admin-head button:hover { color: var(--accent); border-color: var(--accent); }
  .admin-head button.close:hover { color: #ff6666; border-color: #ff4444; }

  .admin-body {
    flex: 1; overflow-y: auto; padding: 18px;
  }
  .admin-body::-webkit-scrollbar { width: 6px; }
  .admin-body::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 3px; }
  ...
  html[data-theme="light"] .admin-section { background: rgba(0,0,0,.02); }
  .admin-section h4 {
    color: var(--accent); font-size: .75em;
  ...
  .admin-row { display: flex; align-items: center; justify-content: space-between; margin-bottom: 10px; gap: 10px; }
  .admin-row label { color: #888; font-size: .78em; letter-spacing: 1px; flex: 1; }
  html[data-theme="light"] .admin-row label { color: #666; }
  .admin-row input[type="range"] {
    flex: 1.5; accent-color: var(--accent); height: 4px;
  }
  .admin-row .val {
    color: var(--accent); font-size: .75em; font-weight: bold;
    min-width: 44px; text-align: right;
  }

  .admin-btn-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; }
  .admin-btn {
    padding: 9px 10px;
    background: rgba(255,255,255,.03);
    border: 1px solid var(--border);
    border-radius: 7px;
    color: #999; font-family: inherit; font-size: .74em;
    letter-spacing: 1px; cursor: pointer;
    transition: .25s; text-align: center;
  }
  html[data-theme="light"] .admin-btn { background: #fafafa; color: #666; }
  ...
  .admin-btn.wide { grid-column: 1 / -1; }
  ...
  .symbol-palette { display: flex; flex-wrap: wrap; gap: 5px; }
  .symbol-btn {
    width: 32px; height: 32px; border-radius: 6px;
    background: rgba(255,255,255,.04);
    border: 1px solid var(--border);
    color: var(--accent); cursor: pointer; font-size: 1em;
    transition: .2s; display: flex; align-items: center; justify-content: center;
  }
  .symbol-btn:hover { background: rgba(var(--accent-rgb), .15); border-color: var(--accent); transform: scale(1.1); }
  ...
  .admin-footer button {
    padding: 12px;
    border-radius: 8px; border: none; cursor: pointer;
    font-family: inherit; font-weight: bold; letter-spacing: 1.5px; font-size: .8em;
    transition: .25s;
  }
  .admin-footer .save-btn { background: var(--accent); color: #000; }
  .admin-footer .save-btn:hover { box-shadow: 0 0 25px rgba(var(--accent-rgb), .6); }
  .admin-footer .reset-btn { background: rgba(255,255,255,.05); color: #888; border: 1px solid var(--border); }
  .admin-footer .reset-btn:hover { color: #ff6666; border-color: #ff4444; }

  /* Визуальная пометка редактируемых элементов */
  body.admin-active [contenteditable="true"]:hover {
    outline: 2px dashed rgba(var(--accent-rgb), .5);
    outline-offset: 2px;
    background: rgba(var(--accent-rgb), .04);
  }
  body.admin-active [contenteditable="true"]:focus {
    outline: 2px solid var(--accent);
    outline-offset: 2px;
    background: rgba(var(--accent-rgb), .08);
  }
  body.admin-active .editable-block { position: relative; }
  body.admin-active .editable-block::before {
    content: '✏'; position: absolute; top: -10px; left: -10px;
    width: 22px; height: 22px; border-radius: 50%;
    background: var(--accent); color: #000;
    display: flex; align-items: center; justify-content: center;
    font-size: .7em; font-weight: bold; z-index: 15;
    pointer-events: none;
  }
  ...
  .header {
    ...
    border-bottom: 3px solid var(--accent); margin-bottom: 50px;
  ...
  @keyframes scanHeader { 0% { left: -60%; } 60%, 100% { left: 120%; } }
  .classification {
    display: inline-block; background: #ff2a2a; color: #fff;
    padding: 8px 25px; font-weight: bold; letter-spacing: 4px;
    font-size: .9em; margin-bottom: 25px;
    border-radius: 4px; box-shadow: 0 0 30px rgba(255,0,0,.45);
    animation: blink 2s infinite, levelGlitch 5s infinite;
  }
  h1 {
    font-size: clamp(2em, 6vw, 4em);
    background: linear-gradient(90deg, var(--accent), var(--accent-dim), var(--accent-deep), var(--accent));
    background-size: 300% auto;
    -webkit-background-clip: text; background-clip: text;
    -webkit-text-fill-color: transparent;
    letter-spacing: 5px; margin-bottom: 15px;
    animation: gradientShift 6s linear infinite;
    filter: drop-shadow(0 0 25px rgba(var(--accent-rgb), .25));
  }
  ...
  h2 {
    color: var(--accent); font-size: clamp(1.3em, 3vw, 1.9em);
  ...
  h3 {
    color: var(--accent-dim); font-size: clamp(1.05em, 2.2vw, 1.35em);
    margin: 35px 0 18px; padding-left: 18px;
    border-left: 4px solid var(--accent-dim); letter-spacing: 1px;
  ...
  .two-col-grid, .code-grid, .clearance-grid {
    ...
    gap: var(--grid-gap); margin: 18px 0; width: 100%; align-items: start;
  }
  ...
  html[data-theme="light"] .data-table { background: #ffffff; }
  .data-table th {
    background: rgba(var(--accent-rgb), .08); color: var(--accent);
    padding: 14px 16px; text-align: left; font-weight: bold;
    border-bottom: 2px solid var(--accent); text-transform: uppercase; font-size: .85em;
  }
  ...
  html[data-theme="light"] .data-table td { background: #ffffff; color: #333; border-color: #ececec; }
  ...
  .table-wrap {
    ...
    margin: 14px 0; width: 100%; border: 1px solid var(--border);
  }
  html[data-theme="light"] .table-wrap { background: #ffffff; border-color: #e0e0e0; }

  .code-card {
    padding: calc(22px * var(--card-scale)); border-radius: 12px;
    background: var(--panel-2); border: 2px solid;
    transition: .35s var(--ease); position: relative; overflow: hidden;
  }
  html[data-theme="light"] .code-card { background: #ffffff; }
  ...
  .code-red h4 { color: #ff0000; }
  .code-black { border-color: #444; background: linear-gradient(135deg, #0f0f0f, rgba(50,50,50,.15)); }
  .code-black::before { background: #444; }
  .code-black h4 { color: #999; }
  .code-green { border-color: var(--accent); background: linear-gradient(135deg, #0f0f0f, rgba(var(--accent-rgb), .08)); }
  .code-green::before { background: var(--accent); }
  .code-green h4 { color: var(--accent); }
  ...
  .code-superblue h4 { color: #00ccff; }
  ...
  .code-superclean h4 { color: #ff69b4; }
  ...
  .section {
    margin: 24px 0; padding: calc(24px * var(--card-scale));
    background: var(--panel); border-radius: 12px;
    border: 1px solid var(--border);
  }
  html[data-theme="light"] .section { background: #ffffff; border-color: #e0e0e0; }
  ...
  html[data-theme="light"] .footer { background: #ffffff; color: #777; }
  ...
  .discord:hover { background: #4752c4; transform: translateY(-3px) scale(1.04); }
  .telegram { background: #0088cc; }
  .telegram:hover { background: #006699; transform: translateY(-3px) scale(1.04); }
  .youtube { background: #ff0000; }
  .youtube:hover { background: #cc0000; transform: translateY(-3px) scale(1.04); }
  ...
  html[data-theme="light"] .acc { background: #ffffff; border-color: #e0e0e0; }
  .acc-head {
    width: 100%; text-align: left; cursor: pointer;
    color: var(--accent); font-family: inherit; font-size: 1em;
  ...
  .clearance-card {
    padding: calc(22px * var(--card-scale)); border-radius: 12px; border: 2px solid;
    background: var(--panel-2); transition: .35s var(--ease);
  }
  html[data-theme="light"] .clearance-card { background: #ffffff; }
  ...
  html[data-theme="light"] .scp-card { background: #ffffff; }
...
<body>
  ...
  <!-- КНОПКА АДМИНКИ (выше поиска, левее) -->
  ...
  <div class="modal-backdrop" id="modalBackdrop">
    <div class="modal">
      <h3>🔐 Админ-панель</h3>
      <p class="modal-sub">Введите пароль для доступа</p>
  ...
  <aside class="admin-panel" id="adminPanel">
    <div class="admin-head">
      <h3>⚙ Админ-панель</h3>
      <div class="actions">
        <button id="adminUndo" title="Отменить (Ctrl+Z)">↶</button>
        <button id="adminRedo" title="Вернуть (Ctrl+Y)">↷</button>
        <button class="close" id="adminClose" title="Закрыть">✕</button>
      </div>
    </div>
    ...
    <div class="admin-body">
      ...
      <div class="admin-section">
        ...
        <div class="admin-btn-grid">
          <button class="admin-btn" data-theme="dark">🌙 Тёмная</button>
          <button class="admin-btn" data-theme="light">☀ Светлая</button>
        </div>
        <div class="color-grid" id="adminColorGrid" style="margin-top:12px;"></div>
      </div>

      <div class="admin-section">
        <h4>📐 Размеры блоков</h4>
        <div class="admin-row">
          <label>Ширина контента</label>
          <input type="range" id="ctrlContentWidth" min="60" max="100" step="1" value="100">
          <span class="val" id="valContentWidth">100%</span>
        </div>
        <div class="admin-row">
          <label>Паддинг карточек</label>
          <input type="range" id="ctrlCardScale" min="0.6" max="2" step="0.05" value="1">
          <span class="val" id="valCardScale">1.00x</span>
        </div>
      </div>
      ...
      <div class="admin-section">
        <h4>📏 Плотность</h4>
        <div class="admin-btn-grid">
          <button class="admin-btn" data-density="compact">Компактно</button>
          <button class="admin-btn" data-density="normal">Обычно</button>
          <button class="admin-btn wide" data-density="spacious">Просторно</button>
        </div>
      </div>
      ...
      <div class="admin-section">
        <h4>🔤 Шрифт</h4>
        <div class="admin-btn-grid">
          <button class="admin-btn" data-font="mono">Mono</button>
          <button class="admin-btn" data-font="sans">Sans</button>
          <button class="admin-btn" data-font="serif">Serif</button>
          <button class="admin-btn" data-font="cyber">Cyber</button>
        </div>
      </div>
      ...
      <div class="admin-section">
        <h4>🌌 Фон</h4>
        <div class="admin-btn-grid">
          <button class="admin-btn" data-bgfx="default">Пусто</button>
          <button class="admin-btn" data-bgfx="grid">Сетка</button>
          <button class="admin-btn" data-bgfx="noise">Шум</button>
          <button class="admin-btn" data-bgfx="gradient">Градиент</button>
          <button class="admin-btn wide" data-bgfx="space">Космос</button>
        </div>
      </div>
      ...
      <div class="admin-section">
        <h4>✏ Режим правки</h4>
        <div class="admin-btn-grid">
          <button class="admin-btn wide" id="toggleEditMode">Включить правку текста</button>
        </div>
        <p style="color:#666; font-size:.72em; margin-top:8px; line-height:1.5;">
          Включи — и все заголовки, абзацы и пункты списков станут редактируемыми прямо на странице. Кликай и правь.
        </p>
      </div>

      <div class="admin-section">
        <h4>🔣 Вставить символ</h4>
        <div class="symbol-palette" id="symbolPalette"></div>
        <p style="color:#666; font-size:.72em; margin-top:8px; line-height:1.5;">
          Символ вставится в последнее активное поле или скопируется в буфер.
        </p>
      </div>
    ...
    <div class="admin-footer">
      <button class="save-btn" id="adminSave">💾 Сохранить HTML</button>
      <button class="reset-btn" id="adminReset">↺ Сбросить всё</button>
      <button class="reset-btn" id="adminExit">🚪 Выйти</button>
    </div>
  </aside>
  ...
  <main class="main-content">
    <div class="main-inner">
      ...
      <div class="clearance-grid">
        <div class="clearance-card level-1 reveal-scale">
          <h4>УРОВЕНЬ 1</h4>
        ...
        <div class="clearance-card level-2 reveal-scale">
          <h4>УРОВЕНЬ 2</h4>
        ...
        <div class="clearance-card level-3 reveal-scale">
          <h4>УРОВЕНЬ 3</h4>
        ...
        <div class="clearance-card level-4 reveal-scale">
          <h4>УРОВЕНЬ 4</h4>
        ...
        <div class="clearance-card level-5 reveal-scale">
          <h4>УРОВЕНЬ 5</h4>
      ...
      <div class="two-col-grid">
        <div class="priv-card reveal-scale" id="priv-obligations">
        ...
        <div class="priv-card reveal-scale" id="priv-forbidden" style="border-color:#ff0000;">
        ...
        <div class="priv-card reveal-scale" id="priv-lies" style="border-color:#ffaa00;">
        ...
        <div class="priv-card reveal-scale" id="priv-confidential" style="border-color:#ff0000;">
        ...
        <div class="priv-card reveal-scale" id="priv-others" style="border-color:#9013fe;">
        ...
        <div class="priv-card reveal-scale" id="priv-hierarchy" style="border-color:#00ff88;">
        ...
        <div class="priv-card reveal-scale" id="priv-punish" style="border-color:#f5a623;">
        ...
        <div class="priv-card reveal-scale" id="priv-rights" style="border-color:#00ccff;">
      ...
      <div class="two-col-grid">
        <div class="item-card item-arrest reveal-scale">
          <h4>⚠️ Арест + вывод на эвакуацию</h4>
          <p style="color:#ffaa00; font-size:.9em;">При обнаружении запрещённых предметов:</p>
          <ul>
            <li>Обнаружено <strong>запрещённое оружие</strong>.</li>
          ...
          <ol>
            ...
            <li><strong>Вывести на эвакуацию</strong>.</li>
          </ol>
        </div>
      ...
      <div class="two-col-grid">
        <div class="item-card item-execute reveal-scale">
      ...
      <div class="two-col-grid">
        <div class="section reveal-scale" style="margin:0;">
      ...
      <p class="reveal">Подробная информация о каждом SCP: здоровье, разумность, способности.</p>
      ...
      <div class="two-col-grid">
        <div class="scp-card reveal-scale">
          ...
          <ul>
            <li><strong>Скачок</strong> — телепорт до 8 м при зрительном контакте.</li>
            <li><strong>Лужа грязи</strong> — замедляет людей (F).</li>
            <li>Сопротивление пулям (кроме Micro H.I.D.).</li>
          </ul>
          <p style="color:#88ffbb;"><strong>Можно:</strong> двигаться при отсутствии зрительного контакта.</p>
          <p style="color:#ff6666;"><strong>Нельзя:</strong> телепортироваться при 3+ наблюдателях.</p>
        </div>
        ...
        <div class="scp-card reveal-scale">
          ...
          <ul>
            ...
            <li><strong>Воскрешение</strong> — поднимает мёртвых как SCP-049-2.</li>
          </ul>
          <p style="color:#88ffbb;"><strong>Можно:</strong> лечить, воскрешать зомби по RP.</p>
          <p style="color:#ff6666;"><strong>Нельзя:</strong> убивать всех подряд.</p>
        </div>
        ...
        <div class="scp-card reveal-scale">
          ...
          <p style="color:#88ffbb;"><strong>Можно:</strong> захватывать людей по RP.</p>
          <p style="color:#ff6666;"><strong>Нельзя:</strong> отправлять в карман без RP.</p>
        </div>
        ...
        <div class="scp-card reveal-scale" style="border-color: #ffaa00;">
          ...
          <p style="color:#ffcc66;"><strong>Статус:</strong> SCP-096 эвакуирован из Участка 11 и переведён в другой объект Фонда.</p>
          <p style="color:#ffcc66;"><strong>Причина:</strong> Угроза признана слишком высокой для содержания на Участке 11.</p>
          <p style="color:#ff8888;"><strong>Внимание:</strong> Если вы заметили SCP-096 — сообщите администрации.</p>
        </div>

        <div class="scp-card reveal-scale">
          <h4>👄 SCP-939 — Многоголосый</h4>
          <div class="hp-badge">❤️ 7 000 HP</div>
          <p><strong>Разумность:</strong> ⚠️ Полуразумный</p>
          <ul>
            <li><strong>Мимикрия</strong> — имитирует голоса людей.</li>
            <li><strong>Укус</strong> — 65 урона + амнезия.</li>
            <li>Чувствительность к звуку (видит сквозь стены).</li>
          </ul>
          <p style="color:#ff6666;"><strong>Нельзя:</strong> кемперить, игнорировать правила.</p>
        </div>
        ...
        <div class="scp-card reveal-scale">
          ...
          <p style="color:#ff6666;"><strong>Нельзя:</strong> убивать без RP-причины.</p>
        </div>

        <div class="scp-card reveal-scale">
          <h4>💻 SCP-079 — Старый ИИ</h4>
          <div class="hp-badge">❤️ 0 HP (уязвим к перегрузке)</div>
          <p><strong>Разумность:</strong> ✅ Да (ИИ)</p>
          <ul>
        ...
        <div class="scp-card reveal-scale" id="scp953">
          ...
          <p style="color:#ff6666;"><strong>Нельзя:</strong> использовать облик для RDM.</p>
        </div>
      </div>
  ...
  <script>
    ...
    const ADMIN_PASSWORD = 'MV-Admin-2026'; // ← ПОМЕНЯЙ ПАРОЛЬ ЗДЕСЬ
    ...
    function snapshot() {
      ...
      // Обрезаем "будущее" если что-то отменили и сделали новое действие
    ...
    function applySnapshot(snap) {
      // Заменяем содержимое head и body без перезагрузки
      const parser = new DOMParser();
      const doc = parser.parseFromString(snap.html, 'text/html');
      document.head.innerHTML = doc.head.innerHTML;
      document.body.innerHTML = doc.body.innerHTML;
      // Пере-подключаем всю логику после восстановления
      setTimeout(() => { rebindAll(); }, 20);
    }
    function undo() {
    ...
    const adminRedo = document.getElementById('adminRedo');
    ...
    function unlockAdmin() {
      ...
      localStorage.setItem('mvp-admin-unlocked', '1');
      snapshot();
    }
    function lockAdmin() {
      ...
      localStorage.removeItem('mvp-admin-unlocked');
      setEditMode(false);
    }

    adminOpenBtn.addEventListener('click', () => {
      if (adminOpenBtn.classList.contains('unlocked')) {
    ...
    adminExit.addEventListener('click', lockAdmin);

    // ---------- Тема и цвета ----------
    const COLORS = [
      { id: 'green', hex: '#00ff88' }, { id: 'cyan', hex: '#00ccff' },
      { id: 'blue', hex: '#4a90d9' }, { id: 'purple', hex: '#b366ff' },
      { id: 'magenta', hex: '#ff44dd' }, { id: 'pink', hex: '#ff69b4' },
      { id: 'red', hex: '#ff4444' }, { id: 'orange', hex: '#ff8800' },
      { id: 'amber', hex: '#ffaa00' }, { id: 'yellow', hex: '#ffdd00' },
      { id: 'lime', hex: '#aaff00' }, { id: 'white', hex: '#ffffff' }
    ];
    const adminColorGrid = document.getElementById('adminColorGrid');
    COLORS.forEach(c => {
      ...
      adminColorGrid.appendChild(b);
    });
    function setAccent(id) {
      state.accent = id;
      html.dataset.accent = id;
      document.querySelectorAll('.color-swatch').forEach(s =>
        s.classList.toggle('active', s.dataset.accent === id));
      saveLocal(); snapshot();
    }
    function setTheme(id) {
      state.theme = id;
      html.dataset.theme = id;
      document.querySelectorAll('.admin-btn[data-theme]').forEach(b =>
    ...
    // ---------- Плотность ----------
    document.querySelectorAll('.admin-btn[data-density]').forEach(b => {
      b.addEventListener('click', () => {
        state.density = b.dataset.density;
        html.dataset.density = b.dataset.density;
        document.querySelectorAll('.admin-btn[data-density]').forEach(x =>
          x.classList.toggle('active', x.dataset.density === state.density));
        saveLocal(); snapshot();
      });
    });

    // ---------- Шрифт ----------
    document.querySelectorAll('.admin-btn[data-font]').forEach(b => {
      b.addEventListener('click', () => {
        state.font = b.dataset.font;
        html.dataset.font = b.dataset.font;
        document.querySelectorAll('.admin-btn[data-font]').forEach(x =>
    ...
    // ---------- Фон ----------
    document.querySelectorAll('.admin-btn[data-bgfx]').forEach(b => {
      b.addEventListener('click', () => {
        state.bgfx = b.dataset.bgfx;
        html.dataset.bgfx = b.dataset.bgfx;
        document.querySelectorAll('.admin-btn[data-bgfx]').forEach(x =>
          x.classList.toggle('active', x.dataset.bgfx === state.bgfx));
        saveLocal(); snapshot();
      });
    });

    // ---------- Ползунки ----------
    const ctrlContentWidth = document.getElementById('ctrlContentWidth');
    const valContentWidth = document.getElementById('valContentWidth');
    const ctrlCardScale = document.getElementById('ctrlCardScale');
    const valCardScale = document.getElementById('valCardScale');

    ctrlContentWidth.addEventListener('input', () => {
      state.contentWidth = ctrlContentWidth.value;
      html.style.setProperty('--content-width', ctrlContentWidth.value + '%');
      valContentWidth.textContent = ctrlContentWidth.value + '%';
    });
    ctrlContentWidth.addEventListener('change', () => { saveLocal(); snapshot(); });

    ctrlCardScale.addEventListener('input', () => {
      state.cardScale = parseFloat(ctrlCardScale.value);
      html.style.setProperty('--card-scale', ctrlCardScale.value);
      valCardScale.textContent = parseFloat(ctrlCardScale.value).toFixed(2) + 'x';
    });
    ctrlCardScale.addEventListener('change', () => { saveLocal(); snapshot(); });

    // ---------- Режим правки текста ----------
    const EDITABLE_SELECTOR = 'h1, h2, h3, h4, p, li, td, th, strong, .subtitle, .codename, .classification, .highlight';
    ...
    function setEditMode(on) {
      ...
      toggleEditMode.classList.toggle('active', on);
      toggleEditMode.textContent = on ? '✅ Правка включена' : 'Включить правку текста';
    }
    ...
    SYMBOLS.forEach(sym => {
      ...
      b.textContent = sym;
      b.addEventListener('click', () => {
        if (lastActiveField && lastActiveField.isContentEditable) {
          const sel = window.getSelection();
          if (sel.rangeCount && lastActiveField.contains(sel.anchorNode)) {
            ...
            lastActiveField.textContent += sym;
          }
        } else if (lastActiveField && (lastActiveField.tagName === 'INPUT' || lastActiveField.tagName === 'TEXTAREA')) {
          const start = lastActiveField.selectionStart || 0;
          const end = lastActiveField.selectionEnd || 0;
          const v = lastActiveField.value;
          lastActiveField.value = v.slice(0, start) + sym + v.slice(end);
          lastActiveField.selectionStart = lastActiveField.selectionEnd = start + sym.length;
          lastActiveField.focus();
        } else {
          navigator.clipboard && navigator.clipboard.writeText(sym);
        }
        snapshot();
      });
    ...
    function saveLocal() {
      ...
      localStorage.setItem('mvp-settings', JSON.stringify(data));
    }
    function loadLocal() {
      try {
        const raw = localStorage.getItem('mvp-settings');
        if (!raw) return;
        const d = JSON.parse(raw);
        if (d.accent) setAccent(d.accent);
        if (d.theme) setTheme(d.theme);
        if (d.font) {
          state.font = d.font; html.dataset.font = d.font;
          document.querySelectorAll('.admin-btn[data-font]').forEach(x =>
            x.classList.toggle('active', x.dataset.font === d.font));
        }
        if (d.density) {
          state.density = d.density; html.dataset.density = d.density;
          document.querySelectorAll('.admin-btn[data-density]').forEach(x =>
            x.classList.toggle('active', x.dataset.density === d.density));
        }
        if (d.bgfx) {
          state.bgfx = d.bgfx; html.dataset.bgfx = d.bgfx;
          document.querySelectorAll('.admin-btn[data-bgfx]').forEach(x =>
            x.classList.toggle('active', x.dataset.bgfx === d.bgfx));
        }
        if (d.contentWidth) {
          ctrlContentWidth.value = d.contentWidth;
          html.style.setProperty('--content-width', d.contentWidth + '%');
          valContentWidth.textContent = d.contentWidth + '%';
        }
        if (d.cardScale) {
          ctrlCardScale.value = d.cardScale;
          html.style.setProperty('--card-scale', d.cardScale);
          valCardScale.textContent = parseFloat(d.cardScale).toFixed(2) + 'x';
        }
      } catch (e) {}
    }
    ...
    // ---------- Undo / Redo кнопки ----------
    adminUndo.addEventListener('click', undo);
    adminRedo.addEventListener('click', redo);
    ...
    adminSave.addEventListener('click', () => {
      // Синхронизируем все inline-правки в разметку
      const clone = document.documentElement.cloneNode(true);

      // Убираем разметку админки и её состояние
      clone.querySelectorAll('#adminPanel, #modalBackdrop, #adminOpenBtn, #searchResults, #searchBox').forEach(el => el.remove());
      // Убираем контентные подсказки
      clone.querySelectorAll('[contenteditable]').forEach(el => {
        el.removeAttribute('contenteditable');
        el.removeAttribute('spellcheck');
      });
      clone.querySelectorAll('.color-swatch.active, .admin-btn.active').forEach(el => el.classList.remove('active'));
      clone.querySelectorAll('#watermark-overlay, #layer-back, #layer-middle, #layer-front').forEach(el => el.innerHTML = '');

      // Оставляем только нужные data-атрибуты у <html>
      const htmlAttrs = clone.attributes;
      const keepAttrs = ['lang', 'data-theme', 'data-accent', 'data-font', 'data-density', 'data-bgfx'];
      Array.from(htmlAttrs).forEach(a => {
        if (!keepAttrs.includes(a.name)) clone.removeAttribute(a.name);
      });
      // Убираем inline-стили, которые ставил ползунок
      clone.style.removeProperty('--content-width');
      clone.style.removeProperty('--card-scale');

      const doctype = '<!DOCTYPE html>\n';
      const htmlStr = doctype + clone.outerHTML;
      ...
      note.textContent = '✅ Файл index.html скачан — загрузи его в репозиторий';
      note.style.cssText = 'position:fixed;top:20px;left:50%;transform:translateX(-50%);background:#00ff88;color:#000;padding:12px 24px;border-radius:8px;font-weight:bold;z-index:9999;box-shadow:0 8px 30px rgba(0,255,136,.5);font-family:monospace;letter-spacing:1px;';
    ...
    adminReset.addEventListener('click', () => {
      if (!confirm('Сбросить все настройки и перезагрузить страницу?')) return;
      localStorage.removeItem('mvp-settings');
      localStorage.removeItem('mvp-admin-unlocked');
      location.reload();
    });
    ...
    // ---------- Автовосстановление сессии админа ----------
    if (localStorage.getItem('mvp-admin-unlocked') === '1') {
      setTimeout(() => {
        adminOpenBtn.classList.add('unlocked');
        adminOpenBtn.textContent = '⚙';
        document.body.classList.add('admin-active');
    ...
    // ---------- Ре-биндинг после undo ----------
    function rebindAll() {
      // Пере-инициализируем аккордеоны
      document.querySelectorAll('.acc-head').forEach(head => {
        head.onclick = () => head.parentElement.classList.toggle('open');
      });
      // Пере-инициализируем навигацию
      document.querySelectorAll('.nav-section-header').forEach(h => {
        h.onclick = () => toggleSection(h);
      });
      // Обновляем состояние режима правки
      if (state.editMode) setEditMode(true);
      // Обновляем undo-состояние кнопок
      loadLocal();
    }

    /* ============================================================
       БАЗОВЫЙ САЙТ (поиск, прогресс, аккордеон, watermark, parallax)
       ============================================================ */
    ...
    (function createWatermarks() {
      ...
      if (!overlay) return;
      const cols = 5, rows = 9, text = 'MV.PROJECT';
    ...
    (function parallaxScroll() {
      ...
      const middleEl = document.getElementById('layer-middle');
      if (!backEl) return;
      let currentBack = 0, currentMiddle = 0;
      function animate() {
    ...
    (function initSearch() {
      ...
      function render(query) {
        ...
        if (matches.length === 0) {
          ...
          empty.innerHTML = 'Ничего не найдено: <strong>' + escapeHTML(query) + '</strong>';
      ...
      function goTo(el) {
        ...
        el.style.backgroundColor = 'rgba(var(--accent-rgb), .15)';
        el.style.boxShadow = '0 0 30px rgba(var(--accent-rgb), .4)';
        setTimeout(() => { el.style.backgroundColor = ''; el.style.boxShadow = ''; }, 1400);
      }
      function closeSearch() { searchResults.classList.remove('show'); searchInput.blur(); }
      ...
      searchInput.addEventListener('keydown', e => {
        ...
        if (e.key === 'ArrowDown') { e.preventDefault(); if (!items.length) return; activeIdx = (activeIdx + 1) % items.length; updateActive(); return; }
        if (e.key === 'ArrowUp') { e.preventDefault(); if (!items.length) return; activeIdx = activeIdx <= 0 ? items.length - 1 : activeIdx - 1; updateActive(); return; }
        if (e.key === 'Enter') { e.preventDefault(); const t = activeIdx >= 0 ? items[activeIdx] : items[0]; if (t) t.click(); }
      });
      searchClear.addEventListener('click', () => {
        searchInput.value = ''; searchBox.classList.remove('has-value');
        searchResults.classList.remove('show'); searchInput.focus();
      });
    ...
    // Стартовый снапшот и первичный запуск
    setTimeout(() => { snapshot(); onScroll(); }, 300);
